...
from src.risk_aggregator import _compute_risk_scores, _decide
...
# Keep the learned-model evidence separate from the existing preprocessing /
# OCR / validation / classical tampering pipeline. The latter is already in
# `result`; the specialized model gets its own score below.
...
# Build a small, serializable model-evidence object for the risk aggregator.
# YOLO contributes its strongest localized confidence; MobileNet contributes
# its forged probability. No model result is mixed into the old tampering
# issue list, so the two evidence sources remain auditable independently.
if selected_model_type in {"sidtd_yolo", "visa_yolo"}:
    specialized_model_data.update({
        "type": "yolo",
        "model_name": (
            "SIDTD grouped YOLO"
            if selected_model_type == "sidtd_yolo"
    ...
    specialized_model_data.update({
        ...
        "status": mobilenet_status,
        "forged_probability": mobilenet_forged or 0.0,
        "bona_fide_probability": mobilenet_bona_fide or 0.0,
    })

# Re-score now that the specialized model has actually run. `result.risk_score`
# from screen_document() is the existing/preprocessing score; the helper below
# calculates both components and the final combined score.
pre_risk, model_risk, combined_risk = _compute_risk_scores(
    result.issues,
    result.tampering.tamper_score,
    specialized_model_data,
)

# If no specialized model was selected or it failed to run, preserve the old
# pipeline's score instead of accidentally multiplying it by 0.60.
if selected_model_type is None or specialized_model_data.get("status") not in {"ok"}:
    combined_risk = pre_risk

result.preprocessing_risk_score = pre_risk
result.model_risk_score = model_risk
result.risk_score = combined_risk
result.model_data = specialized_model_data
result.decision, result.rationale = _decide(result.risk_score, result.issues)
...
if decision == Decision.CLEAR:
    st.success(f"✅ Decision: **CLEAR**   |   Combined Risk: {risk:.1f} / 100")
elif decision == Decision.REFER:
    st.warning(f"⚠️  Decision: **REFER**   |   Combined Risk: {risk:.1f} / 100")
else:
    st.error(f"⛔ Decision: **REJECT**   |   Combined Risk: {risk:.1f} / 100")
...
r1.metric("Preprocessing risk", f"{result.preprocessing_risk_score:.1f} / 100")
r2.metric("Model risk", f"{result.model_risk_score:.1f} / 100")
r3.metric("Combined risk", f"{result.risk_score:.1f} / 100")
st.caption(
    "Preprocessing = existing image/OCR/validation/tampering signals. "
    "Model = specialized YOLO/MobileNet signal. Combined = 60% preprocessing + 40% model when a specialized model runs."
)
...
with st.expander("Raw result (JSON)"):
    ...
    payload["specialized_model"] = specialized_model_data
